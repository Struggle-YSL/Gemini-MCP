...
test("runOrchestratorLoop submits ready gemini work and keeps codex actions visible", async () => {
  ...
  assert.deepEqual(result.codex_actions.map((item) => item.work_item_id), ["backend-1"]);
...
test("runOrchestratorLoop persists submitted task bindings to SQLite snapshots", async () => {
  const tempRoot = mkdtempSync(path.join(tmpdir(), "gemini-mcp-orchestrator-loop-"));
  ...
  assert.equal(stored.snapshot.state.task_bindings[0]?.task_id, "task-frontend-code-1");
  assert.equal(stored.snapshot.state.task_bindings[0]?.work_item_id, "frontend-code-1");
});