...
export const planFrontendSolutionInputSchema = z.object({
  ...
  scope: z.array(z.string()).min(1).describe("前端变更范围，如页面、组件、样式、交互"),
  constraints: z.array(z.string()).optional().describe("前端限制条件"),
  backend_contracts: z.array(z.string()).optional().describe("后端接口约束或数据结构说明"),
  acceptance_criteria: z.array(z.string()).optional().describe("验收标准"),
  project_context: requiredProjectContextSchema,
  session_id: z.string().optional().describe("可选，用于多轮前端规划上下文复用"),
});
...
export const implementFrontendTaskInputSchema = z.object({
  ...
  related_files: z.array(relatedFileSchema).optional().describe("与该任务相关的已有文件上下文"),
  allowed_paths: z.array(z.string()).min(1).describe("Gemini 可返回的文件路径白名单"),
  backend_contracts: z.array(z.string()).optional().describe("前后端接口契约"),
  acceptance_criteria: z.array(z.string()).optional().describe("子任务验收标准"),
  project_context: requiredProjectContextSchema,
  session_id: z.string().optional().describe("可选，用于连续前端编码上下文复用"),
});
...
export const frontendPatchOutputSchema = z.object({
  ...
  progress_stage: z.enum(["queued", "prompting", "generating", "packaging", "completed", "failed"]).optional(),
  files: z.array(frontendPatchFileSchema),
  validation_steps: z.array(z.string()),
  open_questions: z.array(z.string()),
  risks: z.array(z.string()),
...
export type PlanFrontendSolutionInput = z.infer<typeof planFrontendSolutionInputSchema>;
...
export type ImplementFrontendTaskInput = z.infer<typeof implementFrontendTaskInputSchema>;
