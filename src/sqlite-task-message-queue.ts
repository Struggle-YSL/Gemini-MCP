import type { QueuedMessage, TaskMessageQueue } from "@modelcontextprotocol/sdk/experimental/tasks/interfaces.js";
...
export class SQLiteTaskMessageQueue implements TaskMessageQueue {
  ...
  async enqueue(taskId: string, message: QueuedMessage, _sessionId?: string, maxSize?: number): Promise<void> {
    const countRow = this.db.prepare("SELECT COUNT(*) AS count FROM task_messages WHERE task_id = ?").get(taskId) as { count: number };
    if (maxSize !== undefined && countRow.count >= maxSize) {
      throw new Error(`Task message queue overflow: queue size (${countRow.count}) exceeds maximum (${maxSize})`);
    }
