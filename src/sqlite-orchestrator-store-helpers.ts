...
export const ORCHESTRATOR_SELECT_COLUMNS = "orchestrator_id, graph_json, state_json, summary_json, context_json, runtime_json, events_json, final_summary_json, updated_at";

export function parseOrchestratorRow(row: OrchestratorRow): OrchestratorSnapshot {
  return {
    ...
    context: deserializeJson<PersistedOrchestratorContext | undefined>(row.context_json, undefined),
    runtime: deserializeJson<PersistedOrchestratorRuntimeState | undefined>(row.runtime_json, undefined),
    events: deserializeJson<OrchestratorEvent[] | undefined>(row.events_json, undefined),
    final_summary: deserializeJson<OrchestratorFinalSummary | undefined>(row.final_summary_json, undefined),
...
function isTerminalOrchestratorSnapshot(snapshot: OrchestratorSnapshot): boolean {
  return snapshot.state.work_items.every((item) => item.status === "completed" || item.status === "failed");
}

export function isRecoverableOrchestratorSnapshot(snapshot: OrchestratorSnapshot): boolean {
  return !isTerminalOrchestratorSnapshot(snapshot)
